...
## Retrieve test images for testing prediction accuracy:
...
# loop over some guesses for C and gamma

C_range = np.logspace(0, 10, 15, base=2)
gamma_range = np.logspace(-7, 4, 15, base=2)

tcorrects = []
icorrects = []

for Cv in C_range:
    for gammav in gamma_range:
        svm = None
        svm = cv2.SVM()
        ...
        tcorrects.append(ncorrect_text)
        icorrects.append(ncorrect_image)
        print Cv, gammav
